...
player. */
function SetName() {
  // Fetch data from the name input screen.
  name = document.getElementById("playerName").value;
  var nameInput = document.getElementById("playerName").value;

    // Name conditions (between 2 and 12 chars long, no spaces)
    if (nameInput.length > 12) {
      ...
      hue = document.getElementById("hueSelect").value;
      console.log("Hue set to " + hue);

      // Stop imgTimer interval
      function stopTimer() {
        clearInterval(hueRefresh);
      }

      // Save the image inside canvas.
      // Not working atm, will return later.
      /*
      var exportCanvas = document.getElementById("imgPreview");
      var image = exportCanvas.toDataURL("image/png").replace("image/png", "image/octet-stream");
      window.location.href = image;
      */

      // Initialise Player(), passing the name from text box.
